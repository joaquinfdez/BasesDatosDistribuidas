...
BEGIN
  ...
  CASE 
    ...
    WHEN xDelegacion = 'Madrid' THEN 
      INSERT INTO panfleto951.empleado 
    ...
    CASE
        ...
        IF (xCont<>0) THEN
          ...
          IF (xCont > 0) THEN
            ...
            IF (xDelegacion = 'Madrid') THEN -- SI LA DELEGACIÓN ES MADRID
              UPDATE panfleto951.sucursal SET director = null WHERE director = xcodempleado;
            END IF;
            IF (xDelegacion = 'Barcelona') THEN -- SI LA DELEGACIÓN ES BARCELONA
              UPDATE panfleto952.sucursal SET director = null WHERE director = xcodempleado;
            END IF;
            IF (xDelegacion = 'La Coruña') THEN -- SI LA DELEGACIÓN ES LA CORUÑA
              UPDATE panfleto953.sucursal SET director = null WHERE director = xcodempleado;
            END IF;
            IF (xDelegacion = 'Sevilla') THEN -- SI LA DELEGACIÓN ES SEVILLA
              UPDATE panfleto954.sucursal SET director = null WHERE director = xcodempleado;
            END IF;
            DBMS_OUTPUT.PUT_LINE('LA SUCURSAL DE LA QUE ERA DIRECTOR HA QUEDADO VACÍA');
          END IF;
        END IF;
        DELETE FROM panfleto951.empleado WHERE codEmpleado = xCodEmpleado;
      ...
      WHEN xDelegacion = 'Barcelona' THEN 
        ...
        IF (xCont<>0) THEN
          ...
          END IF;
        END IF;
        DELETE FROM panfleto952.empleado WHERE codEmpleado = xcodempleado;
      ...
      WHEN xDelegacion = 'La Coruña' THEN 
        ...
        IF (xCont<>0) THEN
          ...
          END IF;
        END IF;
        DELETE FROM panfleto953.empleado WHERE codEmpleado = xcodempleado;
      ...
      WHEN xDelegacion = 'Sevilla' THEN 
        ...
        IF (xCont<>0) THEN
          ...
          END IF;
        END IF;
        DELETE FROM panfleto954.empleado WHERE codEmpleado = xcodempleado;
    END CASE;
...
BEGIN 
  ...
  ELSE
    ...
    CASE
      -- SI LA DELEGACIÓN PERTENECE A MADRID
      WHEN xDelegacion = 'Madrid' THEN 
        UPDATE panfleto951.empleado SET salario = xSalario WHERE codEmpleado = xCodEmpleado;
        
      -- SI LA DELEGACIÓN PERTENECE A BARCELONA
      WHEN xDelegacion = 'Barcelona' THEN 
        UPDATE panfleto952.empleado SET salario = xSalario WHERE codEmpleado = xCodEmpleado;
      
      -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
      WHEN xDelegacion = 'La Coruña' THEN 
        UPDATE panfleto953.empleado SET salario = xSalario WHERE codEmpleado = xCodEmpleado;
          
      -- SI LA DELEGACIÓN PERTENECE A SEVILLA
      WHEN xDelegacion = 'Sevilla' THEN
        UPDATE panfleto954.empleado SET salario = xSalario WHERE codEmpleado = xCodEmpleado;
    END CASE;
...
BEGIN
  ...
  CASE
			-- SI LA DELEGACIÓN PERTENECE A MADRID
			WHEN xDelegacion = 'Madrid' THEN
  			INSERT INTO panfleto951.sucursal 
        VALUES (xCodSucursal, xNombre, xCCAA, xCiudad, xDirector, xDelegacion);
      
			-- SI LA DELEGACIÓN PERTENECE A BARCELONA
			WHEN xDelegacion = 'Barcelona' THEN 
  			INSERT INTO panfleto952.sucursal 
        VALUES (xCodSucursal, xNombre, xCCAA, xCiudad, xDirector, xDelegacion);
		  
			-- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
			WHEN xDelegacion = 'La Coruña' THEN 
  			INSERT INTO panfleto953.sucursal 
        VALUES (xCodSucursal, xNombre, xCCAA, xCiudad, xDirector, xDelegacion);
			  
			-- SI LA DELEGACIÓN PERTENECE A SEVILLA
			WHEN xDelegacion = 'Sevilla' THEN
  			INSERT INTO panfleto954.sucursal 
        VALUES (xCodSucursal, xNombre, xCCAA, xCiudad, xDirector, xDelegacion);
		END CASE;
...
BEGIN
  ...
  CASE
    -- SI LA DELEGACIÓN PERTENECE A MADRID
    WHEN xDelegacion = 'Madrid' THEN
      UPDATE panfleto951.sucursal SET director = xCodEmpleado WHERE codSucursal = xCodSucursal;
    
    -- SI LA DELEGACIÓN PERTENECE A BARCELONA
    WHEN xDelegacion = 'Barcelona' THEN 
      UPDATE panfleto952.sucursal SET director = xCodEmpleado WHERE codSucursal = xCodSucursal;
    
    -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
    WHEN xDelegacion = 'La Coruña' THEN 
      UPDATE panfleto953.sucursal SET director = xCodEmpleado WHERE codSucursal = xCodSucursal;
      
    -- SI LA DELEGACIÓN PERTENECE A SEVILLA
    WHEN xDelegacion = 'Sevilla' THEN
      UPDATE panfleto954.sucursal SET director = xCodEmpleado WHERE codSucursal = xCodSucursal;
  END CASE;
...
BEGIN
  ...
  ELSE
    ...
    CASE
      -- SI LA DELEGACIÓN PERTENECE A MADRID
      WHEN xDelegacion = 'Madrid' THEN 
        INSERT INTO panfleto951.cliente VALUES (xCodCliente, xDNI, xNombre, xDireccion, xTipoCliente, xCCAA);
        
      -- SI LA DELEGACIÓN PERTENECE A BARCELONA
      WHEN xDelegacion = 'Barcelona' THEN 
        INSERT INTO panfleto952.cliente VALUES (xCodCliente, xDNI, xNombre, xDireccion, xTipoCliente, xCCAA);
      
      -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
      WHEN xDelegacion = 'La Coruña' THEN 
        INSERT INTO panfleto953.cliente VALUES (xCodCliente, xDNI, xNombre, xDireccion, xTipoCliente, xCCAA);
          
      -- SI LA DELEGACIÓN PERTENECE A SEVILLA
      WHEN xDelegacion = 'Sevilla' THEN
        INSERT INTO panfleto954.cliente VALUES (xCodCliente, xDNI, xNombre, xDireccion, xTipoCliente, xCCAA);
    END CASE;
...
BEGIN
  ...
  -- DISMINUIMOS EL STOCK DISPONIBLE DE ESE VINO EN SU LOCALIDAD.
    IF xDelegacionVino = 'Madrid' THEN
      UPDATE panfleto951.vino SET stock = stock - xCantidad WHERE codVino = xCodVino;
    ELSE 
      IF xDelegacionVino = 'Barcelona' THEN
        UPDATE panfleto952.vino SET stock = stock - xCantidad WHERE codVino = xCodVino;
      ELSE 
        IF xDelegacionVino = 'La Coruña' THEN
          UPDATE panfleto953.vino SET stock = stock - xCantidad WHERE codVino = xCodVino;
        ELSE 
          UPDATE panfleto954.vino SET stock = stock - xCantidad WHERE codVino = xCodVino;
        END IF;
      END IF;
    END IF;
    ...
    CASE
      ...
      WHEN xDelegacion = 'Madrid' THEN 
        ...
        UPDATE panfleto951.realizaPedido 
      ...
      WHEN xDelegacion = 'Barcelona' THEN 
        -- SUMAMOS LA CANTIDAD QUE HABÍA A LA QUE SE VUELVE A PEDIR
        UPDATE panfleto952.realizaPedido 
        SET cantidad = cantidad + xCantidad
        WHERE codCliente = xCodCliente 
        AND codSucursal = xCodSucursal 
        AND codVino = xCodVino
      ...
      WHEN xDelegacion = 'La Coruña' THEN 
        -- SUMAMOS LA CANTIDAD QUE HABÍA A LA QUE SE VUELVE A PEDIR
        UPDATE panfleto953.realizaPedido 
        SET cantidad = cantidad + xCantidad
        WHERE codCliente = xCodCliente 
        AND codSucursal = xCodSucursal 
        AND codVino = xCodVino
      ...
      WHEN xDelegacion = 'Sevilla' THEN
        -- SUMAMOS LA CANTIDAD QUE HABÍA A LA QUE SE VUELVE A PEDIR
        UPDATE panfleto954.realizaPedido 
        SET cantidad = cantidad + xCantidad
        WHERE codCliente = xCodCliente 
        AND codSucursal = xCodSucursal 
        AND codVino = xCodVino
  ...
  ELSE  -- NO EXISTE UN PEDIDO DE ESE VINO POR ESE CLIENTE, EN ESA FECHA, A ESA SUCURSAL, 
    ...
    CASE
      ...
      WHEN xDelegacion = 'Madrid' THEN 
        ...
        INSERT INTO panfleto951.realizaPedido 
        VALUES (xCodSucursal, xCodVino, xCodCliente, xFecha, xCantidad);
        
      -- SI LA DELEGACIÓN PERTENECE A BARCELONA
      WHEN xDelegacion = 'Barcelona' THEN 
        -- INSERTAMOS EL NUEVO PEDIDO.
        INSERT INTO panfleto952.realizaPedido 
        VALUES (xCodSucursal, xCodVino, xCodCliente, xFecha, xCantidad);
      
      -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
      WHEN xDelegacion = 'La Coruña' THEN 
        -- INSERTAMOS EL NUEVO PEDIDO.
        INSERT INTO panfleto953.realizaPedido 
        VALUES (xCodSucursal, xCodVino, xCodCliente, xFecha, xCantidad);
          
      -- SI LA DELEGACIÓN PERTENECE A SEVILLA
      WHEN xDelegacion = 'Sevilla' THEN
        -- INSERTAMOS EL NUEVO PEDIDO.
        INSERT INTO panfleto954.realizaPedido 
        VALUES (xCodSucursal, xCodVino, xCodCliente, xFecha, xCantidad);
    ...
    ELSE
      ...
      IF (xFecha IS NULL) THEN -- BORRAMOS TODOS LOS PEDIDOS DE ESE CLIENTE, DE ESE VINO 
        ...
        CASE
          -- SI LA DELEGACIÓN PERTENECE A MADRID
          WHEN xDelegacion = 'Madrid' THEN 
            DELETE FROM panfleto951.realizaPedido 
            WHERE codCliente = xCodCliente 
            AND codSucursal = xCodSucursal 
            AND codVino = xCodVino;
          ...
          -- SI LA DELEGACIÓN PERTENECE A BARCELONA
          WHEN xDelegacion = 'Barcelona' THEN 
            DELETE FROM panfleto952.realizaPedido 
            WHERE codCliente = xCodCliente 
            AND codSucursal = xCodSucursal 
            AND codVino = xCodVino;
          ...
          -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
          WHEN xDelegacion = 'La Coruña' THEN
            DELETE FROM panfleto953.realizaPedido 
            WHERE codCliente = xCodCliente 
            AND codSucursal = xCodSucursal 
            AND codVino = xCodVino;
          ...
          -- SI LA DELEGACIÓN PERTENECE A SEVILLA
          WHEN xDelegacion = 'Sevilla' THEN
            DELETE FROM panfleto954.realizaPedido 
            WHERE codCliente = xCodCliente 
            AND codSucursal = xCodSucursal 
            AND codVino = xCodVino;
        END CASE;
...
BEGIN
  ...
  IF (xCont > 0) THEN -- SI EXISTE UN PEDIDO CON ESA FECHA DE UNA SUCURSAL A OTRA DE ESE VINO, ACTUALIZAMOS.
    ...
    CASE
      ...
      WHEN (xDelegacion = 'Madrid') THEN 
        -- SUMAMOS LA CANTIDAD QUE HABÍA A LA QUE SE VUELVE A PEDIR
        UPDATE panfleto951.pedidoSucursal 
        SET cantidad = cantidad + xCantidad
      ...
      WHEN (xDelegacion = 'Barcelona') THEN 
        -- SUMAMOS LA CANTIDAD QUE HABÍA A LA QUE SE VUELVE A PEDIR
        UPDATE panfleto952.pedidoSucursal 
        SET cantidad = cantidad + xCantidad
        WHERE CodSucPide = xCodSucPide
        AND codSucPedido = xCodSucPedido
        AND vino = xCodVino
      ...
      WHEN (xDelegacion = 'La Coruña') THEN 
        -- SUMAMOS LA CANTIDAD QUE HABÍA A LA QUE SE VUELVE A PEDIR
        UPDATE panfleto953.pedidoSucursal 
        SET cantidad = cantidad + xCantidad
        WHERE CodSucPide = xCodSucPide
        AND codSucPedido = xCodSucPedido
        AND vino = xCodVino
      ...
      WHEN (xDelegacion = 'Sevilla') THEN
        -- SUMAMOS LA CANTIDAD QUE HABÍA A LA QUE SE VUELVE A PEDIR
        UPDATE panfleto954.pedidoSucursal 
        SET cantidad = cantidad + xCantidad
        WHERE CodSucPide = xCodSucPide
        AND codSucPedido = xCodSucPedido
        AND vino = xCodVino
  ...
  ELSE  -- NO EXISTE UN PEDIDO DE ESE VINO POR ESE CLIENTE, EN ESA FECHA A ESA SUCURSAL.
    ...
    CASE
      ...
      WHEN xDelegacion = 'Madrid' THEN 
        -- INSERTAMOS EL NUEVO PEDIDO.
        INSERT INTO panfleto951.pedidoSucursal 
        VALUES (xCodSucPide, xcodSucPedido, xcodVino, xFecha, xCantidad);
        
      -- SI LA DELEGACIÓN PERTENECE A BARCELONA
      WHEN xDelegacion = 'Barcelona' THEN 
        -- INSERTAMOS EL NUEVO PEDIDO.
        INSERT INTO panfleto952.pedidoSucursal 
        VALUES (xCodSucPide, xcodSucPedido, xcodVino, xFecha, xCantidad);
        
      -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
      WHEN xDelegacion = 'La Coruña' THEN 
        -- INSERTAMOS EL NUEVO PEDIDO.
        INSERT INTO panfleto953.pedidoSucursal 
        VALUES (xCodSucPide, xcodSucPedido, xcodVino, xFecha, xCantidad);
        
      -- SI LA DELEGACIÓN PERTENECE A SEVILLA
      WHEN xDelegacion = 'Sevilla' THEN
        -- INSERTAMOS EL NUEVO PEDIDO.
        INSERT INTO panfleto954.pedidoSucursal 
        VALUES (xCodSucPide, xcodSucPedido, xcodVino, xFecha, xCantidad);
  ...
  ELSE
    ...
    IF (xFecha IS NULL) THEN -- BORRAMOS TODOS LOS PEDIDOS DE ESAS SUCURSALES DE ESE VINO
      CASE
        -- SI LA DELEGACIÓN PERTENECE A MADRID
        WHEN xDelegacion = 'Madrid' THEN 
          DELETE FROM panfleto951.pedidoSucursal 
          WHERE CodSucPide = xCodSucPide 
          AND CodSucPedido = xCodSucPedido 
          AND vino = xCodVino;
        ...
        -- SI LA DELEGACIÓN PERTENECE A BARCELONA
        WHEN xDelegacion = 'Barcelona' THEN 
          DELETE FROM panfleto952.pedidoSucursal 
          WHERE CodSucPide = xCodSucPide 
          AND CodSucPedido = xCodSucPedido 
          AND vino = xCodVino;
        ...
        -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
        WHEN xDelegacion = 'La Coruña' THEN
          DELETE FROM panfleto953.pedidoSucursal 
          WHERE CodSucPide = xCodSucPide 
          AND CodSucPedido = xCodSucPedido 
          AND vino = xCodVino;
        ...
        -- SI LA DELEGACIÓN PERTENECE A SEVILLA
        WHEN xDelegacion = 'Sevilla' THEN
          DELETE FROM panfleto954.pedidoSucursal 
          WHERE CodSucPide = xCodSucPide 
          AND CodSucPedido = xCodSucPedido 
          AND vino = xCodVino;
...
BEGIN
  ...
  ELSE
    ...
    CASE
      -- SI LA DELEGACIÓN PERTENECE A MADRID
      WHEN xDelegacion = 'Madrid' THEN 
        INSERT INTO panfleto951.vino VALUES (xCodVino, xMarca, xAnio, xDenominacion, xGraduacion,
          ...
          WHILE CU_SUCURSALES%FOUND LOOP  -- RECORREMOS LAS SUCURSALES DE LA DELEGACIÓN
            INSERT INTO panfleto951.vende VALUES (xCodSucursal, xCodVino); -- TODAS LAS SUCURSALES DE LA DELEGACIÓN VENDERÁN EL VINO
            FETCH CU_SUCURSALES INTO xCodSucursal;
          END LOOP;
       
      -- SI LA DELEGACIÓN PERTENECE A BARCELONA
      WHEN xDelegacion = 'Barcelona' THEN 
        INSERT INTO panfleto952.vino VALUES (xCodVino, xMarca, xAnio, xDenominacion, xGraduacion,
          xVinedo, xCantidad, xCodProductor, xCCAA, xCantidad); -- INSERTAMOS
          OPEN CU_SUCURSALES;
          FETCH CU_SUCURSALES INTO xCodSucursal;
          WHILE CU_SUCURSALES%FOUND LOOP  -- RECORREMOS LAS SUCURSALES DE LA DELEGACIÓN
            INSERT INTO panfleto952.vende VALUES (xCodSucursal, xCodVino);  -- TODAS LAS SUCURSALES DE LA DELEGACIÓN VENDERÁN EL VINO
            FETCH CU_SUCURSALES INTO xCodSucursal;
          END LOOP;
      
      -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
      WHEN xDelegacion = 'La Coruña' THEN 
        INSERT INTO panfleto953.vino VALUES (xCodVino, xMarca, xAnio, xDenominacion, xGraduacion,
          xVinedo, xCantidad, xCodProductor, xCCAA, xCantidad); -- INSERTAMOS
          OPEN CU_SUCURSALES;
          FETCH CU_SUCURSALES INTO xCodSucursal;
          WHILE CU_SUCURSALES%FOUND LOOP  -- RECORREMOS LAS SUCURSALES DE LA DELEGACIÓN
            INSERT INTO panfleto953.vende VALUES (xCodSucursal, xCodVino); -- TODAS LAS SUCURSALES DE LA DELEGACIÓN VENDERÁN EL VINO
            FETCH CU_SUCURSALES INTO xCodSucursal;
          END LOOP;
          
      -- SI LA DELEGACIÓN PERTENECE A SEVILLA
      WHEN xDelegacion = 'Sevilla' THEN
        INSERT INTO panfleto954.vino VALUES (xCodVino, xMarca, xAnio, xDenominacion, xGraduacion,
          xVinedo, xCantidad, xCodProductor, xCCAA, xCantidad); -- INSERTAMOS
          OPEN CU_SUCURSALES;
          FETCH CU_SUCURSALES INTO xCodSucursal;
          WHILE CU_SUCURSALES%FOUND LOOP  -- RECORREMOS LAS SUCURSALES DE LA DELEGACIÓN
            INSERT INTO panfleto954.vende VALUES (xCodSucursal, xCodVino); -- TODAS LAS SUCURSALES DE LA DELEGACIÓN VENDERÁN EL VINO
            FETCH CU_SUCURSALES INTO xCodSucursal;
          END LOOP;
    END CASE;
    CLOSE CU_SUCURSALES;
...
BEGIN
  ...
  SELECT CCAA INTO xCCAA FROM v_vino WHERE codvino = xCodVino;
    ...
    CASE
      -- SI LA DELEGACIÓN PERTENECE A MADRID
      WHEN xDelegacion = 'Madrid' THEN 
        DELETE FROM panfleto951.vende WHERE codVino = xCodVino;
        DELETE FROM panfleto951.vino WHERE codVino = xCodVino;
        
      -- SI LA DELEGACIÓN PERTENECE A BARCELONA
      WHEN xDelegacion = 'Barcelona' THEN 
        DELETE FROM panfleto952.vende WHERE codVino = xCodVino;
        DELETE FROM panfleto952.vino WHERE codVino = xCodVino;
      
      -- SI LA DELEGACIÓN PERTENECE A LA CORUÑA
      WHEN xDelegacion = 'La Coruña' THEN 
        DELETE FROM panfleto953.vende WHERE codVino = xCodVino;
        DELETE FROM panfleto953.vino WHERE codVino = xCodVino;
          
      -- SI LA DELEGACIÓN PERTENECE A SEVILLA
      WHEN xDelegacion = 'Sevilla' THEN
        DELETE FROM panfleto954.vende WHERE codVino = xCodVino;
        DELETE FROM panfleto954.vino WHERE codVino = xCodVino;
...
BEGIN
  INSERT INTO panfleto951.productor VALUES (xCodProductor, xDNI, xNombre, xDireccion);
  INSERT INTO panfleto952.productor VALUES (xCodProductor, xDNI, xNombre, xDireccion);
  INSERT INTO panfleto953.productor VALUES (xCodProductor, xDNI, xNombre, xDireccion);
  INSERT INTO panfleto954.productor VALUES (xCodProductor, xDNI, xNombre, xDireccion);
...
-- COMPROBAMOS SI EXISTE UN PRODUCTOR CON UN VINO EN EL SISTEMA.
  ...
  ELSE
    ...
    END IF;
    DELETE FROM panfleto951.vino WHERE codProductor = xCodProductor;
    DELETE FROM panfleto952.vino WHERE codProductor = xCodProductor;
    DELETE FROM panfleto953.vino WHERE codProductor = xCodProductor;
    DELETE FROM panfleto954.vino WHERE codProductor = xCodProductor;
